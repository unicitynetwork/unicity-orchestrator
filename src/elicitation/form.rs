...
use crate::elicitation::{
    ...
    ElicitationSchema, StringFormat,
};
...
fn get_enum_values(enum_schema: &EnumSchema) -> Vec<&str> {
    match enum_schema {
        EnumSchema::Single(single) => match single {
            ...
            SingleSelectEnumSchema::Titled(s) => {
                s.one_of.iter().map(|c| c.const_.as_ref()).collect()
            }
        },
        EnumSchema::Multi(multi) => match multi {
            MultiSelectEnumSchema::Untitled(s) => {
                s.items.enum_.iter().map(|v| v.as_str()).collect()
            }
            MultiSelectEnumSchema::Titled(s) => {
                s.items.any_of.iter().map(|c| c.const_.as_ref()).collect()
            }
        },
...
impl FormHandler {
        ...
        match response.action {
            ElicitationAction::Accept => {
                let content = response.content.as_ref().ok_or_else(|| {
                    ElicitationError::InvalidSchema("Response must include content".to_string())
                })?;
    ...
    fn validate_content_against_schema(
        ...
        let content_obj = content.as_object().ok_or_else(|| {
            ElicitationError::InvalidSchema("Content must be an object".to_string())
        })?;
        ...
        match schema {
            PrimitiveSchema::String(string_schema) => {
                let s = value.as_str().ok_or_else(|| {
                    ElicitationError::InvalidSchema(format!("Property '{}' must be a string", name))
                })?;
            ...
            PrimitiveSchema::Number(number_schema) => {
                let n = value.as_f64().ok_or_else(|| {
                    ElicitationError::InvalidSchema(format!("Property '{}' must be a number", name))
                })?;
            ...
            PrimitiveSchema::Integer(int_schema) => {
                let n = value.as_i64().ok_or_else(|| {
                    ElicitationError::InvalidSchema(format!(
            ...
            PrimitiveSchema::Enum(enum_schema) => {
                ...
                if !valid_values.contains(&s) {
                    return Err(ElicitationError::InvalidSchema(format!(
                        "Property '{}' has invalid enum value: {}",
                        name, s
                    )));
                }
            }
        ...
        match format {
            StringFormat::Email => {
                ...
                if !value.contains('@') || !value.contains('.') {
                    return Err(ElicitationError::InvalidSchema(format!(
                        "Property '{}' is not a valid email address",
                        name
                    )));
                }
            }
            ...
            StringFormat::Date => {
                ...
                if chrono::NaiveDate::parse_from_str(value, "%Y-%m-%d").is_err() {
                    return Err(ElicitationError::InvalidSchema(format!(
                        "Property '{}' is not a valid date (expected YYYY-MM-DD)",
                        name
                    )));
                }
            }
            StringFormat::DateTime => {
                ...
                if chrono::DateTime::parse_from_rfc3339(value).is_err() {
                    return Err(ElicitationError::InvalidSchema(format!(
                        "Property '{}' is not a valid date-time (expected ISO 8601)",
                        name
                    )));
                }
            }
...
mod tests {
    ...
    fn test_validate_enum_values() {
        ...
        assert!(
            handler
                .validate_response(&schema, &invalid_response)
                .is_err()
        );
    }
}
